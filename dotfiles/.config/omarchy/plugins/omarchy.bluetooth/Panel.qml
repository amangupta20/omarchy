...
Panel {
  ...
  function toggleBluetooth() {
    var turnOn = !(root.adapter && root.adapter.enabled)
    if (turnOn) {
      Quickshell.execDetached(["omarchy-bluetooth-device", "on"])
    } else {
      Quickshell.execDetached(["omarchy-bluetooth-device", "off"])
    }
    if (adapter) {
      adapter.enabled = turnOn
      if (turnOn) Qt.callLater(function() {
